...
public class Carousel{
	...
	public String  carouselfeature(Utilities utility){
		...
		HashMap<String, WearableTech> wearabletechs = new HashMap<String, WearableTech>();
		...
		if(CategoryName==null){
			try{
			wearabletechs=MySqlDataStoreUtilities.getWearableTechs();
		...
		for(String user: prodRecmMap.keySet())
		{
			if(user.equals(utility.username()))
			{
				...
				sb.append("<a style='font-size: 24px;margin-left: auto; margin-right: auto'>Recommended Products</a>");
				...
				*/
				...
				sb.append("<body background-color:#632; margin:0; padding:0; font-family: Open Sans, sans-serif; font-size: 1.0em; color: #889;font-weight: 200;>");
				// sb.append("<link rel='stylesheet' href='styles.css' type='text/css' />");
				...
				*/
				...
				for(String prod : productsList){
					...
					if (k==0 )
					{
						...
						sb.append("<div class='item active'><div class='col-md-6' style = 'background-color:black;border :1px solid #cfd1d3'>");
						
					}
					else
					{
						sb.append("<div class='item'><div class='col-md-6' style = 'background-color: black ;border :1px solid #cfd1d3' >");
					...
					sb.append("<li><form method='post' action='WriteReview'>"+"<input type='hidden' name='name' value='"+prodObj.getId()+"'>"+
							"<input type='hidden' name='type' value='"+prodObj.getType()+"'>"+
							"<input type='hidden' name='maker' value='"+prodObj.getRetailer()+"'>"+
							"<input type='hidden' name='access' value='"+" "+"'>"+
							"<input type='hidden' name='price' value='"+prodObj.getPrice()+"'>"+
							"<input type='submit' value='WriteReview' class='btnreview'></form></li>");
					sb.append("<li><form method='post' action='ViewReview'>"+"<input type='hidden' name='name' value='"+prodObj.getId()+"'>"+
							"<input type='hidden' name='type' value='"+prodObj.getType()+"'>"+
							"<input type='hidden' name='maker' value='"+prodObj.getRetailer()+"'>"+
							"<input type='hidden' name='access' value='"+" "+"'>"+
